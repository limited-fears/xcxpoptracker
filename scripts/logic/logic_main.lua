...
xenoblade_x_location = {}
xenoblade_x_location.__index = xenoblade_x_location
...
function xenoblade_x_location.new(name)
    local self = setmetatable({}, xenoblade_x_location)
    if name then
...
function xenoblade_x_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        exit = xenoblade_x_location.new(exit)
    end
    if rule == nil then
        rule = always
    end
...
function xenoblade_x_location:connect_two_ways(exit, rule)
...
function xenoblade_x_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function xenoblade_x_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function xenoblade_x_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function xenoblade_x_location:accessibility()
...
-- 
function xenoblade_x_location:discover(accessibility, keys)
...
entry_point = xenoblade_x_location.new("entry_point")
